...
pipeline.on('files.recurse.found', handler.foundFile, true);
pipeline.on('files.recurse.end', handler.endRecurse, true);
pipeline.on('files.recurse.changed', handler.changedFile, true);
...
module.exports.$$createInstance = function(root, config, callback) {
  ...
  root.recursor([config.testDir, config.sourceDir], {createDir: true}).then(
    function() {
      callback(null, {});
    },
    function(e) {
