...
var TODO, debug, dev, error, fs, normalize, promise, ref, ref1, reporters, sep, sequence, shared, tester;

ref = require('path'), normalize = ref.normalize, sep = ref.sep;

ref1 = objective.logger, TODO = ref1.TODO, debug = ref1.debug, error = ref1.error;
...
module.exports = dev = shared.dev = {
  ...
  init: function(callback) {
    ...
    pipe = objective.pipe;
    files = {};
    pipe.on('files.recurse.found', function(data) {
    ...
    waiting = [];
    pipe.on('files.recurse.end', function(arg, next) {
      var file, path, type;
      path = arg.path;
      ...
      return sequence((function() {
        ...
        return pipe.emit('multiple.done', {}, function() {
          objective.noRoot = false;
          return next();
        });
      });
    });
    pipe.on('dev.test.after.all', function() {
      try {
        return waiting.pop()();
      } catch (_error) {}
    });
    pipe.on('files.recurse.changed', function(arg, next) {
      var path;
      path = arg.path;
      if (path.match(new RegExp("^" + dev.testDir))) {
        if (objective.currentChild.filename != null) {
