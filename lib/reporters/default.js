...
module.exports.report = function(args, counter, file) {
  ...
  for (var i = 0; i < errors.length; i++) {
    var file = errors[i].test.node.info.file;
    file = file.replace(process.cwd() + sep, '');
    file += ':' + errors[i].test.node.info.line + ':' + errors[i].test.node.info.colm;
    var place = (i+1) + ') ';
    var space = '             '.substr(0, place.length) 
    console.log(EOL + place + errors[i].test.node.path.join(' - '));
    console.log(space + file.grey);
    console.log(space + errors[i].test.node.error.toString().red);
    if (dev.roots[args.root.config.uuid].config.fullTrace) {
      var stack = errors[i].test.node.error.stack.split(EOL);
      stack.shift();
