...
Reporter.prototype.onAfterEach = function(args, next) {
  ...
  // console.log('error?', testNode);
                                // catch after's failing
  if (testNode.error || testNode.cancelled) {

    if (testNode.cancelled) {
      testNode.error = testNode.cancelled.error;
    }

    // console.log('ERROR!');

    if (!testNode.cancelled) {
      if (testNode.error.message == '__pending') counts.pend++
      else counts.fail++;
    }
      ...
      if (this.failures.length == 0) this.failures.push(args)
        ...
        if (previousError.name == 'HookError') {
          if (previousError.cancelled.atStep.node.id != testNode.error.cancelled.atStep.node.id) {
            if (testNode.error.message != '__pending') this.failures.push(args);
          } else {
            if (!testNode.skip) previousError.affected++;
          }
        } else {
          if (testNode.error.message != '__pending') this.failures.push(args);
        }
...
Reporter.prototype.showFailures = function(args, failures, local) {
  ...
  for (var i = 0; i < failures.length; i++) {
    ...
    if (typeof filterTrace !== 'undefined' && filterTrace) showTrace = true
...
Reporter.prototype.showStackTrace = function(args, filterTrace, error, indent, shift, perFrameFn) {
  ...
  stack.forEach(function(line) {
    ...
    if (frame && frame.FileName) {
      path = relative(process.cwd(), frame.FileName);
    ...
    if (filterTrace) {
      ...
      if (path && path.indexOf('node_modules') == -1 && path.indexOf(sep) != -1 /* && path.indexOf(home + sep + 'node_modules') == -1 */) {
