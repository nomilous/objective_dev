...
Reporter.prototype.showFailures = function(args, failures, local) {
  ...
  for (var i = 0; i < failures.length; i++) {
    ...
    if (error.name == 'HookError') {
      ...
      console.log(indent + file.grey);
      console.log(indent + error.error.toString().red);
      error = error.error;
    } else {
      info = test.node.info;
      file = info.file;
    ...
    filterTrace = this.overrideFilterTrace(args, failures[i]);
    if (typeof filterTrace !== 'undefined') showTrace = true;
    if (showTrace || filterTrace) this.showStackTrace(args, filterTrace, error, indent);
    console.log();
  }
}

Reporter.prototype.overrideFilterTrace = function(args, failure) {
...
Reporter.prototype.showStackTrace = function(args, filterTrace, error, indent) {
  ...
  if (filterTrace) {
    regex = new RegExp('^' + home + sep + '(' + config.sourceDir + '|' + config.testDir + ')');
  }
  i = 0;
  stack.shift();
  console.log();
  stack.forEach(function(line) {
    ...
    var frame = frames[i];
    if (filterTrace) {
      if (m = frame.getFileName().match(regex)) {
        line = line.replace(home, rootIndex).trim();
        line = line.replace(/^at\s/, 'at ' + i + ' ');
        console.log(indent + line.grey);
      } 
      i++;
      return;
    }
