...
module.exports.load = function(root, config, argName) {
  ...
  var devconfig = dev.roots[root.config.uuid].config; //same as config?
  ...
  if (true) {
    ...
    try {
      ...
      var startPath = normalize(root.home + sep + config.filename);
      ...
      module.paths = build(dirname(startPath),[]);
