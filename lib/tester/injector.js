...
pipeline.on('dev.test.after.each',
  ...
  for (var key in mocks) {
    ...
    recurse(currentNode);
    if (remove) {
      debug('partial delete mock \'%s\'', key);
      delete mocks[key];
    }
  }
}, true);
...
module.exports.load = function(root, config, argName) {
  ...
  if (argName == 'Module') {
    return module.exports.mock(argName, require(root.home), require.resolve(root.home));
  }

  else if (argName.match(/^[A-Z]/)) {
