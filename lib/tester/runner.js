...
// TOTO  - number test failures (reporter)
// TODO  - show failures only at the end
// TODO  - option to run all tests on each change
//         and .only behaves appropriately
...
pipeline.on('objective.multiple.end', 'runAll', 
  ...
  sequence(
    Object.keys(args.root.children).map(function(uuid) {
      return function() {
        return promise(function(resolve, reject) {
          var tree = args.root.children[uuid].tree;
          var childArgs = {
            root: args.root,
          ...
          module.exports.run(deferral, childArgs, tree);
        });
      }
    }
  )
