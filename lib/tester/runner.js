// TODO  - afterhooks still run on beforehook fail
// TODO  - still run unaffected tests if before hook fails, each affected test fails with hook error
                  ...
                  if (step.node.cancelled) {
                    ...
                    if (cancelledAt.node.id == step.node.id) {
                      if (step.type == 'afterAll' ) {
                        if (cancelledAt.type == 'beforeAll') {
                          runAnyway = true; // Still run afterAll for failed beforeAll (only if on same testnode)
                        }
                      }
                    }
...
module.exports.createDone = function(root, config, currentStep, testContext, resolve, reject) {
  ...
  return done = function(error) {
    ...
    if (count > 1) {
      var e = new Error('Done called ' + count + ' times.');

      if (currentStep.step.node.reported) {
        e.message += ' (LATE CALL)';
        // already reported on test step, this done is late...
        var file = currentStep.step.node.info.file;
        file = file.replace(process.cwd() + sep, '');
        file += ':' + currentStep.step.node.info.line + ':' + currentStep.step.node.info.colm;
        console.log();
        console.log('?) ' + currentStep.step.node.path.join(' - '));
        console.log('   ' + file.grey);
        console.log('   ' + e.toString().red);
        if (dev.roots[root.config.uuid].config.fullTrace) {
          var stack = e.stack.split(EOL);
          stack.shift();
        ...
        currentStep.step.node.error = new Error('Done called ' + count + ' times.');
      }
      return;
    }
    currentStep.step.endAt = Date.now();
...
module.exports.runStep = function(root, config, testContext, step, resolve, reject) {
  ...
  try {
    ...
    if (step.type != 'test') {
      if (module.exports.cancelBranch(step, e)) {
        return resolve();
      }
      return reject(e);
    }
...
module.exports.cancelBranch = function(step, e) {

  if (step.type == 'afterAll' || step.type == 'afterEach') {
    return false;
  }
