...
module.exports.assignSkip = function(tree) {
  ...
  var recurse = function(node, parents) {
    ...
    if (node.only) {
      if (node.type == 'context') {
        node.skip = false;
        return; // TODO: rather only enable entire context if no only's inside
      } else {
  ...
  return promise(function(resolve, reject, notify) {
    ...
    var recurse = function(parent, undepth) {
      ...
      ['beforeEach','beforeAll'].forEach(function(type){
        for (var i = parent.hooks[type].length - 1; i >= 0; i--) {
          var hook = parent.hooks[type][i];
          if (type == 'beforeAll' && hook.fn.runCount > 0) continue;
          ...
          steps.map(function(step) {
            return function() {
              return promise(function(resolve, reject, notify) {
                ...
                // Quick fix for not running afterAll's (something wrong with isLastSibling);

                if (step.type == 'test') {
                  if (testNode.pend || testNode.skip) return resolve();
                } else {
                  // console.log(step.node.path, testNode.path);
                  // console.log('step/node', step.node.skip, step.node.pend, step.node.path);
                  // console.log('testNode', testNode.skip, testNode.pend, testNode.path);
                  if (step.node.skip || step.node.pend || step.node.cancelled) return resolve();
                }
...
module.exports.isLastSibling = function(node, doRecurse) {
  ...
  if (lastInParent == null) {
    return true;
    // if (!doRecurse) return true;
    // if (node.parent.type == 'root') return !doRecurse; // um?
  } else {
...
module.exports.getLastNotSkippedOrPending = function(nodes, i) {
  ...
  var node = nodes[i];
  if (i == -1) return null;
  if (node.type == 'test' && (node.skip || node.pend)) return module.exports.getLastNotSkippedOrPending(nodes, i - 1);
  return node;
}
...
module.exports.createDone = function(root, config, currentStep, testContext, resolve, reject) {
  ...
  currentStep.timeout = setTimeout(onTimeout = function(){
    ...
    if (currentStep.step.type !== 'test') {
      if (module.exports.cancelBranch(root, currentStep.step, e)) {
        currentStep.step.node.error = new HookError('Error in ' + currentStep.step.type, {
          cancelled: {
            atStep: currentStep.step,
            error: e
          }
        });
        return resolve();
  ...
  return currentStep.done = function(error) {
    if (error instanceof Error) {
      if (currentStep.step.type != 'test') {
        var e = new HookError('Error in ' + currentStep.step.type, {
          cancelled: {
            atStep: currentStep.step,
            error: error
          }
        ...
        currentStep.step.node.error = e;
        module.exports.cancelBranch(root, currentStep.step, e);
      } else {
