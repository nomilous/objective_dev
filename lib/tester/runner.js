...
module.exports.wait = function() {
  ...
  var detail = {};
  var file = objective.getCallerFileName(2, detail);
  file = file.replace(process.cwd() + sep, '');
  warn('Test waiting (10 seconds) at %s:%s:%s',file,detail.lineNumber,detail.columnNumber);
  dev.runStep.waiting = {};

  var timeout = setTimeout(function() {
    // wait 4 seconds for the repl before moving on
    dev.runStep.resolve();
  }, 10000);
...
module.exports.runStep = function(root, config, testContext, step, resolve, reject) {
  ...
  // --attach (objective.plugins.dev.runStep)
  ...
  if (!done && !dev.runStep.waiting) return resolve();
